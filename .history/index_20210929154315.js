...
//   умеет добавлять, удалять , заменять елементы массива. Возвращает массив удаленных или добавленых елементов
...
// sort - сортирует елементы приводя их к строке и сравнивая эти строки в порядке следования символов unicode. Возвращает отсортированный массив(изменяет изначальный массив )
...
// reduce - вычисление единого значения массива. На каждой итерации запоминает значение и просто на следующей его плюсует. Возвращает а
...
// 1) accumulator - сохраняет значения внутри себя. Изначально равен initialValue
// 2) currentValue  - Текущий обрабатываемый элемент массива
// 3) index (необязательный параметр ) - индекс массива пребираемого
// 4) array  (необязательный параметр ) -  массив для которого была вызвана функция reduce
// 5) initialValue (необязательный параметр ) - Объект, используемый в качестве первого аргумента при первом вызове функции callback.
...
// нам нужен thisArg если мы хотим использовать this функции иначе как и понимаю он будет равен window
let army = {
  minAge: 18,
  maxAge: 27,
  canJoin(user) {
