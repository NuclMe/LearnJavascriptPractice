...
function Calculator() {
  ...
  function calculate(str) {
    if (typeof str === "string") {
      let arr = str.split(" ");
      let res = operators[arr[1]](+arr[0], +arr[2]);
    }
  }
  calculate("1 - 1");

  this.addMethod = (name, func) => {
    let result = (operators[name] = func);
    return result;
    debugger;
    // debugger;
    // console.log(result);
...
const testCalc = sum.addMethod("+", (4, 6));
...
//   }
// );
let ask = (questions, yes, no) => {connfirm(questions) ? yes};
const res = yes ? () => alert("вы согласились") : () => alert("вы отказались");
